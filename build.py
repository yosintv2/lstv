...
for i in range(7):
    ...
    day_matches.sort(key=lambda x: (x.get('league_id') not in TOP_LEAGUE_IDS, x.get('league', 'Other')))
    ...
    for m in day_matches:
        ...
        if league != last_league:
            # Insert AD after the previous league block ends
            if last_league != "":
...
# --- 4. GENERATE CHANNELS & MATCH PAGES (logic from previous steps) ---
# ... (Omitted for brevity, use existing channel/match loop)

print("Build Complete with Ads.")
