...
for i in range(7):
    ...
    # Generate Weekly Menu for this page
    ...
    # Filter pool for matches strictly on THIS local date
    day_matches = []
    for m in all_matches_pool:
    ...
    # Sort: Top Leagues -> League Name -> Kickoff Time
    ...
    for m in day_matches:
        ...
        for c in m.get('tv_channels', []):
            chan_pills = "".join([f'<a href="{DOMAIN}/channel/{slugify(ch)}/" style="display: inline-block; background: #f1f5f9; color: #2563eb; padding: 2px 8px; border-radius: 4px; margin: 2px; text-decoration: none; font-weight: 600; border: 1px solid #e2e8f0;">{ch}</a>' for ch in c['channels']])
            
            # Map matches to channels for Channel Pages (Strictly Upcoming)
            for ch in c['channels']:
                if ch not in channels_data: channels_data[ch] = []
                if int(m['kickoff']) > (NOW.timestamp() - 7200): # Filter: Only future or live (within 2hrs)
            ...
            <div style="display: flex; align-items: flex-start; padding: 12px; border-bottom: 1px solid #edf2f7; background: #fff;">
                ...
                <div style="flex: 1; display: flex; flex-wrap: wrap; gap: 4px;">{chan_pills}</div>
            </div>'''
    ...
    # Save daily index/date files
    with open(fname, "w", encoding='utf-8') as df:
        output = templates['home'].replace("{{MATCH_LISTING}}", listing_html or "<p class='p-4 text-center'>No matches scheduled for this date.</p>")
        output = output.replace("{{WEEKLY_MENU}}", page_specific_menu).replace("{{DOMAIN}}", DOMAIN)
...
for ch_name, matches in channels_data.items():
    ...
    # Standard Menu for Channel pages
    chan_menu = f'{MENU_CSS}<div class="weekly-menu-container">'
    for j in range(7):
        m_day = MENU_START_DATE + timedelta(days=j)
        m_fname = "index.html" if m_day == TODAY_DATE else f"{m_day.strftime('%Y-%m-%d')}.html"
        chan_menu += f'<a href="{DOMAIN}/{m_fname}" class="date-btn"><div>{m_day.strftime("%a")}</div><b>{m_day.strftime("%b %d")}</b></a>'
    chan_menu += '</div>'
    ...
    for item in matches:
        ...
        # Home-style UI for channel fixtures
        c_listing += f'''
    ...
    with open(f"{c_dir}/index.html", "w", encoding='utf-8') as cf:
        cf.write(templates['channel'].replace("{{CHANNEL_NAME}}", ch_name).replace("{{MATCH_LISTING}}", c_listing).replace("{{DOMAIN}}", DOMAIN).replace("{{WEEKLY_MENU}}", chan_menu))
...
print(f"Build Complete! {len(all_matches_pool)} matches processed across {len(channels_data)} channels.")
